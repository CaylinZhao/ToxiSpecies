...
def ensemble(setting, args):
    ...
    model_fa = DataAdapter(args, adapters[0]).to(device)
    model_la = DataAdapter(args, adapters[1]).to(device)
    ...
    for split_name in ['valid', 'test']:
        for seed in range(args.eval_seeds):
            ...
            test_da(label_tune, feature_tune, label_eval, feature_eval, model_fa, model_la, args, setting, seed, split_name=split_name)

        df_list = [pd.read_csv('Results/Experiment setting/seeds/DoubleAdapter_Setting_' + str(setting) + '_' + lr_tag + '_' + split_name + '_s' + str(args.runseed) + '_s' + str(seed) + '.csv', index_col=0) for seed in range(args.eval_seeds)]
        combined_df = pd.concat(df_list, axis=0)
        result = combined_df.groupby(combined_df.index).mean()
        result = result.reindex(['R2 score', 'RMSE', 'MAE', 'Pearson', 'Spearman'])
        result = result.round(3)
        result.to_csv('Results/Experiment setting/DoubleAdapter_' + str(setting) + '_' + lr_tag + '_' + split_name + '_s' + str(args.runseed) + '.csv')
